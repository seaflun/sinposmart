...
class QtShellTests(unittest.TestCase):
    ...
    def test_qml_duty_error_bar_and_keyboard_task_contract(self) -> None:
        ...
        self.assertIn("const wasSelected = taskRow.selected", task_area)
        self.assertIn("wasSelected && clearFocusOnDeselect", task_area)
        self.assertIn("Qt.callLater(function() { taskRow.focus = false })", task_area)
        self.assertIn("onClicked: taskRow.activateRow(true)", task_area)
    ...
    def test_qml_shell_loads_with_app_controller(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                self.assertTrue(duty_task_row.property("selectedState"))
                self.assertTrue(duty_task_row.property("activeFocus"))
                click(duty_task_row)
                self.assertEqual(controller.dutyController.selectedTaskCount, 0)
                self.assertFalse(selected_task_actions.property("visible"))
                self.assertFalse(duty_task_row.property("selectedState"))
                self.assertFalse(duty_task_row.property("activeFocus"))
                click(duty_task_row)
                self.assertEqual(controller.dutyController.selectedTaskCount, 1)
                self.assertTrue(selected_task_actions.property("visible"))
