...
1. Google Drive 公開更新檔尚未同步到本機新版。
   - 本機 `UPDATE/VERSION.txt`：`2026.06.03.1614`
...
5. 公務包更新器 ZIP file ID 指到舊檔。
   ...
   - 補強：更新器會在成功或失敗後清理自己建立的 `%TEMP%` 暫存下載/解壓資料；清理失敗只警告，不遮蔽更新結果。
   - 補強：更新器只在遠端版本大於本機版本時更新，避免公開 Drive 舊版導致降版。
   - 補強：更新器會驗證本機、遠端、ZIP 內版本字串格式，避免 HTML 錯誤頁或亂碼被當成版本。
   - 補強：更新器備份 ZIP 改放 `%LOCALAPPDATA%\SinpoSmart\update_backups`，避免備份落在雲端同步專案資料夾。
