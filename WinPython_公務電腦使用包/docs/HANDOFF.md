...
## 2026-09-23 民力到退勤與登出確認

- 每日／每月作業共用左側卡片；救護行車紀錄器與民力系統位於右側卡片。
- 民力登記一次一人，日期與時間可補登；到勤寫入「入／到勤」，退勤寫入「出／退勤」。使用確認時的值班人員帳號，在值班台獨立完成 OA 登入、OCR、SSO、查重、送出及回查，不派發救護 Worker 任務。
- 名冊與常用人員由救護台受既有同步授權保護的唯讀 `/api/sinposmart/civilpower-roster` 取得；離線使用上次成功名冊並標示時間。需先部署救護台對應 API。
- 使用者點擊登出先顯示 AppleDialog；取消不登出，確認才走既有安全登出流程。有已準備更新時保留「登出並更新」與對應說明；自動交接不受手動確認影響。
- 通知捷徑的 Shell COM 生命週期移至專用 QThread，避免後續 QML 視窗載入崩潰；民力確認與執行期間阻擋每日自動工具啟動。
- 發布以 `50b4b98`（2026.09.22.2010）為基準，保留每日自動工具、跨班帳號、一般返隊及遠端更新修正。正式網站未送出測試紀錄。
...
- `legacy_tk/duty_gui.py` 僅保留為舊 Tk 回退介面，不是正式啟動入口，也不會被 Qt 應用 import；根目錄 `duty_gui.py` 是 QML 相容入口。
